...
class ModeloCargos{
	...
	static public function mdlActualizar($tabla, $datos){
		...
		$stmt->bindParam(":id", $datos["idcargo"], PDO::PARAM_INT);
	...
	{
		$stmt = Conexion::conectar()->prepare("SELECT * FROM cargo WHERE id = :id");
	...
	{
		$stmt = Conexion::conectar()->prepare("DELETE FROM cargo WHERE id = :id");
		$stmt->bindParam(":id", $id, PDO::PARAM_INT);
        $stmt->execute();
		return $stmt->fetch(PDO::FETCH_ASSOC);
		$stmt = null; 
